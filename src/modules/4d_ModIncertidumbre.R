...
ModIncertidumbre <- function(VarObj, BaseDatos, rfe_lim, Muestreo, listmodelos){
  ...
  if (is(train.data[,'target'],'numeric')){
    ...
    if (!file.exists(confusionMatrix.archivo.figura)){
      ##### output messages ####
      cat(paste0('### RESULTADO 1 de 5: La figura de la matriz de confusi�n de la variable ',str_replace(VarObj,'[.]','-'),' usando mejor modelo ',modelos.mejor,' NO existe y se esta generando en la ruta ', confusionMatrix.archivo.figura,' ###','\n'))
      ##### end output messages ####
      ...
      ##### output messages ####
      cat(paste0('### RESULTADO 1 de 5: La figura de la matriz de confusi�n de la variable ',str_replace(VarObj,'[.]','-'),' usando mejor modelo ',modelos.mejor,' existe y se encuentra en la ruta ', confusionMatrix.archivo.figura,' ###','\n'))
      ##### end output messages ####
    }
    ...
    if (!file.exists(entropia.archivo.grafica)){
      ...
      limite_shp <- st_read(paste0(in.limite.data,'/aoi'))
