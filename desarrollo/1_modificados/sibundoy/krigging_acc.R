...
## cargar paquetes necesarios y descripción
library(sp)#clases y métodos para datos espaciales
library(raster)#introducción a datos raster en R(es necesario el paquete sp)
library(car)#regresion aplicada
library(rgdal)#enlaces para la bilbioteca de abstracción de datos"geoespacial"
library(gstat)#modelamiento, predicción y simulación geoestadística espacial y espacio temporal
              #modelamiento variograma
library(lattice)#visualización de datos de alto nivel. Necesidades gráficas
library(ggplot2)#costrucción de gráficos
library(caret)#funciones que facilitan el uso méodos complejos de clasificación y regresión
library(reshape)#transformaciones de datos, funciones melt y cast (reorganizar y resumir datos)
library(aplpack)#dibujar gráficos especiales
...
require(MASS)#Retorna FALSE y emite un warning en caso de no poder cargar el paquete. No detiene la eventual ejecución del script.
library(akima)#interpolación de datos espaciados de forma regular e irregular
library(geoR)#herramientas para el análisis de datos geoestadísticos
library(lattice)#repetido
library(maptools)#conjunto de herramientas para manipular datos geográficos

### Se preparan los datos ####
dir()#enumera todos los archivos de un directorio

# Se Cargan los datos de los splines
dat <- read.csv('/Users/ac/Documents/Consultancy/IGAC/projects/3_mapeosuelos/desarrollos/soil-toolbox/proyecto_sibundoy/datos/entrada/1_covariables/color/tabular/RegMatrix_VF.csv', sep=';')#lee los datos
names(dat)#nombre de las variables

### Estructura de los datos
str(dat)#clasificación de variables

#variables culitativas convertidas a un factor
#cambiamos nombres de variables por unos más cortos por conveniencia en programación
...
# caracterización de la variable COS30CM
par(mfrow=c(1,3))#mostrar en pantalla 3 gráficos en una fila
...
lines(density(dat$COS30CM))#línea de densidad de la variable COS30CM

rug(dat$COS30CM,col="blue")#agrega sobre el eje x (variable de interés), los datos del eje x como pequeñas lineas verticales

grid(10,10)#rejilla que se establece sobre un gráfico, sirve para facilitar interpretación en el dibujo

curve(dnorm(x,mean=mean(dat$COS30CM),sd=sd(dat$COS30CM)), col="darkblue",lwd=2,add=T,yaxt="n")#curva de la distribución normal, tomando como media de la campana la COS30CM
...
plot(ecdf(dat$COS30CM),ylab="Frecuencia",xlab="Stock COS (t.ha^-1)",main="COS 30 cm")#grafica la función de densidad acumulada de la variable COS30CM

boxplot(dat$COS30CM,col="lightblue",main="COS 30 cm",ylab="Stock COS (t.ha^-1)")#Boxplot de la variable COS30CM

points(mean(dat$COS30CM),col="red",pch=16)#puntos de la media de la variable COS30CM de color rojo y tamaño 16

## Transformación logarítmica de la variable COS30CM

par(mfrow=c(1,3))##mostrar en pantalla 3 gráficos en una fila

hist(log(dat$COS30CM), prob=T,main="COS 30 cm", xlab="Stock COS (t.ha^-1)",col="lightblue",breaks=100, ylab="Densidad")#histograma de la variable COS30CM aplicándole logaritmo

lines(density(log(dat$COS30CM)))#lineas de densidad de la variable COS30CM, aplicándole log

rug(log(dat$COS30CM),col="blue")#agrega sobre el eje x (variable de interés), los datos del eje x como pequeñas lineas verticales, aplicándole log

grid(10,10)#rejilla que se establece sobre un gráfico, sirve para facilitar interpretación en el dibujo

curve(dnorm(x,mean=mean(log(dat$COS30CM)),sd=sd(log(dat$COS30CM))), col="darkblue",lwd=2,add=T,yaxt="n")#curva de dist. normal,donde la media y la desv. estándar de la campana es el log. de la variable COS30CM

#PENDIENTE#
text(2,0.5,bquote("("*bar(x)* "="*.(round(mean(log(dat$COS30CM)),2))*")"))
#PENDIENTE#

plot(ecdf(log(dat$COS30CM)),ylab="Frecuencia",xlab="Stock COS (t.ha^-1)",main="COS 30 cm")#grafica la función de densidad acumulada del logaritno de la variable COS30CM

boxplot(dat$COS30CM,col="lightblue",main="COS 30 cm",ylab="Stock COS (t.ha^-1)")#Boxplot de la variable COS30CM

points(mean(log(dat$COS30CM)),col="red",pch=16)##puntos de la media del logaritmo de la variable COS30CM de color rojo y tamaño 16
...
COR #matriz de correlación de todos los datos

x <- subset(melt(COR), value != 1 | value!= NA) #x matriz de correlación, excluyendo los valores de 1 y los NA
x <- x[with(x, order(-abs(x$value))),]#define un orden para x
#as.character(x$X2[1:10]) # �qu� es x2?
...
names(dat2)
summary(modelo.MLR) #sumary de la regresión
anova(modelo.MLR) #Analisis de varianza del modelo .MLR
head(dat2$W_1)

## Hacemos seleccion de variables por stepwise
modelo.MLR.step <- step(modelo.MLR, direction="both") #al modelo se le aplica funci� stepwise, hacia atr�s
...
vif(modelo.MLR.step)#Calcula factores de variaci�n-inflaci�n y variaci�n-inflaci�n generalizada para modelos lineales, lineales generalizados, modelo stepwise en este caso
sqrt(vif(modelo.MLR.step)) #se calcula la ra�z cuadrada del vif ultimo
...
#plantear rango intercuart�lico u otro test para calculo de outliers al comienzo del an�lisis, seguido a analisis exploratorio
#en este punto no deber�an ya haber outliers
...
library(raster)#librer�a cargada anteriormente

#install.packages("raster") #instale el paquete r�ster
COV <- stack('/Users/ac/Documents/Consultancy/IGAC/projects/3_mapeosuelos/desarrollos/soil-toolbox/proyecto_sibundoy/datos/entrada/1_covariables/color/raster/COV_SIBU.tif')#Los vectores de apilamiento concatenan m�ltiples vectores en un solo vector junto con un factor que indica d�nde se origin� cada observaci�n
namesCov<- readRDS('/Users/ac/Documents/Consultancy/IGAC/projects/3_mapeosuelos/desarrollos/soil-toolbox/proyecto_sibundoy/datos/entrada/1_covariables/color/rds/NAMES_COV_SIB.rds')# se utiliza la funcion readRDS, funciones para escribir un solo objeto R en un archivo y restaurarlo.
...
COV<- COV[[idx]] #covarianza de idx, vector de caracteres(tama�o 1x5)
#idx <- as.character(x$X2[c(1:5)])

# Project point data ##proyecci�n de los puntos de las base de datos
...
dat2_sp <- spTransform(dat2_sp, CRS("+init=epsg:32618")) #spTransform para proyecci�n de mapas y transformaci�n de sistma de modelos de ref. del elipsoide
...
COV_epsg32618 <- paste0('/Users/ac/Documents/Consultancy/IGAC/projects/3_mapeosuelos/desarrollos/soil-toolbox/proyecto_sibundoy/datos/salida/temp/','COV_epsg32618.tif')
if (!file.exists(COV_epsg32618)){
  COV <- projectRaster(COV, crs = CRS("+init=epsg:32618"), method='ngb')
  writeRaster(COV, filename=COV_epsg32618,overwrite=T)
} else {
  COV <- raster(COV_epsg32618)
}
COV
#COV<- COV[[idx]]
#paquete raster:admite la pryecci�n del vecino m�s cercano (interpolacion)
...
COV.sp <- as(COV, "SpatialGridDataFrame")#asignaci�n de COV a un dataframe espacial
...
library(automap)#Este paquete realiza una interpolaci�n autom�tica estimando autom�ticamente el variograma y luego llamando a gstat
...
print(Sys.time() - start)#esto deber�a dar 0
#start <- Sys.time()


memory.size()> memory.size(TRUE)#asignaci�n de memoria acutual
